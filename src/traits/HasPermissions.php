...
trait HasPermissions
{
     ...
     * @return Query
     */
    public function scopePermission(Query $query, $permissions): Query
    {
        ...
        return $query->where(function (Query $query) use ($permissions, $rolesWithPermissions) {
            $query->whereHas('permissions', function (Query $subQuery) use ($permissions) {
                $subQuery->whereIn(config('permission.table_names.permissions').'.id', \array_column($permissions, 'id'));
            });
            if (count($rolesWithPermissions) > 0) {
                $query->orWhereHas('roles', function (Query $subQuery) use ($rolesWithPermissions) {
