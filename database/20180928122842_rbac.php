...
class Rbac extends Migrator
{
    ...
    public function up()
	{
		$table = $this->table(config('permission.table.role'), [ 'engine'=>'InnoDB']);
		...
		$table = $this->table(config('permission.table.permission'), ['engine' => 'InnoDB']);
		...
		$table = $this->table(config('permission.table.user_has_roles'), ['engine' => 'InnoDB', 'identity' => true]);
		$table->addColumn('uid', 'integer',['limit' => 11, 'comment'=>'用户ID'])
			  ->addColumn('role_id', 'integer', [ 'comment' => '角色ID'])
			  ->create();

		$table = $this->table(config('permission.table.role_has_permissions'), ['engine' => 'InnoDB', 'identity' => true]);
